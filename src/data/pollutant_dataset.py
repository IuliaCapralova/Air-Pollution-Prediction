...
class PollutantDataset(DataSet):
    ...
    #     self._dataframe.loc[:, 'YYYYMMDD'] = pd.to_datetime(self._dataframe['Einddatumtijd']).dt.strftime('%Y%m%d')
    #     self._dataframe.loc[:, 'HH'] = pd.to_datetime(self._dataframe['Einddatumtijd']).dt.strftime('%H')
    ...
    def _lin_interpolation(self, prev_idx:int, next_idx:int, gap_size:int) -> None:
        ...
        # print(f"Gap size: {gap_size}. Previous index: {prev_idx}, value {prev_val}, next index: {next_idx}, value {next_val}")
        ...
        for i in range(prev_idx+1, next_idx):
            ...
            # print(f"LINEAR INTERPOLATION AT {i} with previous value {self._dataframe.iloc[i - 1][self._variable]} and delta {delta}: {self._dataframe.iloc[i][self._variable]}")
